/**
 * 跨进程通信（IPC）服务主入口模块
 * 抽象统一的 IPC 接口门面，自动在 Tauri 原生桌面端与 Web 纯前端模式之间无缝切换
 */

import { initTauriBridge } from "./tauriBridge";
import { initWebBridge } from "./webBridge";
/**
...
export class IPCService {
  public localConfig: LocalDeviceConfig;
  public listeners: Map<string, Set<EventCallback<any>>> = new Map();
  public broadcastChannel: BroadcastChannel | null = null;
  public virtualPeers: PeerDevice[] = [];
  public activeSimulations: Map<string, number> = new Map();
  public webHiddenToTray: boolean = false;
  ...
  public async init(): Promise<void> {
    ...
    this.initPromise = isTauri()
      ? initTauriBridge(this as any)
      : initWebBridge(this as any);
    ...
    if (isTauri()) {
      try {
        ...
        await invoke("set_auto_start", {
          enabled: !!this.localConfig.autoStart,
        });
  ...
  public broadcastLocalHeartbeat() {
  ...
  public handleIncomingPeerHeartbeat(peer: PeerDevice) {
    this.registerDiscoveredPeer(peer);
  }

  public checkPeersLiveness() {
    const now = Date.now();
    let changed = false;
  ...
  async sendChatMessage(
    ...
    msg.senderAvatarUrl = toCompactAvatarIdentifier(
  ...
  public async simulatePeerReply(peer: PeerDevice, userText: string) {
    ...
    const replies: Record<string, string[]> = {
      ...
      speed: [
        `[${peer.name}]: 测试信道速率良好，Tokio 异步 I/O 准备就绪，跑满带宽无瓶颈。`,
      ],
    };

    const textList =
      userText.includes("速") || userText.includes("文件")
        ? replies.speed
        : replies.default;
  ...
  async getPartialFileSize(fileName: string): Promise<number> {
    if (isTauri()) {
      try {
        const { invoke } = await import("@tauri-apps/api/core");
        const size = await invoke<number>("get_partial_file_size", {
          fileName,
        });
        return size || 0;
      } catch (err) {
  ...
  // 从本地硬盘/缓存中物理删除指定文件
  async deleteFileFromDisk(filePath?: string): Promise<boolean> {
    if (!filePath || filePath.trim() === "") return false;
    if (isTauri()) {
      try {
        const { invoke } = await import("@tauri-apps/api/core");
        const res = await invoke<boolean>("delete_file_from_disk", {
          filePath,
        });
        return Boolean(res);
      } catch (err) {
        console.warn("物理删除文件失败:", err);
        return false;
      }
    }
    return true;
  ...
  // 批量物理删除文件
  async deleteFilesFromDisk(filePaths: string[]): Promise<number> {
    if (!filePaths || filePaths.length === 0) return 0;
    if (isTauri()) {
      try {
        const { invoke } = await import("@tauri-apps/api/core");
        const count = await invoke<number>("delete_files_from_disk", {
          filePaths,
        });
        return count || 0;
      } catch (err) {
        console.warn("批量物理删除文件失败:", err);
        return 0;
      }
    }
      ...
      try {
        ...
        return {
          ...
          message:
            typeof err === "string"
              ? err
              : err?.message || "检查更新失败，请确认 Master 机器在线",
        };
      }
    } else {
