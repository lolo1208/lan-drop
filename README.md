...
- **去中心化与零配置（纯 HTTP 发现）**：基于局域网 HTTP 主动温和探测与握手协议（`GET /api/info`），设备接入局域网后自动发现彼此，无需任何第三方中央服务器或账号注册，彻底避免了 UDP 组播在复杂 Wi-Fi AP 隔离或企业防火墙环境下的丢包与阻断限制。
- **极速磁盘直推传输**：底层基于 Rust Tokio 异步运行时、Axum 高性能 HTTP 接收端与 Reqwest 磁盘流式推送，大文件直接流式写入磁盘，内存占用极低，跑满内网千兆/万兆带宽。
...
该模式会同时启动 Vite 前端热重载服务和 Tauri 桌面原生窗口，享有完整的局域网真实 HTTP 设备发现与文件流式传输服务：
...
- 检查系统防火墙是否允许应用通过 TCP `57088` 端口入站通信（若在“系统设置”中自定义了端口，请放行对应端口）；
- 若网络开启了严格子网隔离，可点击左侧边栏顶部的 **“+” 号按钮**，直接输入对端 IP 与端口进行精准 HTTP 握手；
- 支持在“系统偏好设置” -> “系统设置”中自定义服务通信端口（默认 57088）。

---

## 局域网通信协议与端口说明

本工具无任何外部服务器中转，通信全程基于本地局域网 HTTP/TCP 通信（可在系统设置中自定义端口）：

| 通信类型 | 协议 | 默认端口 | 功能说明 |
| :--- | :--- | :--- | :--- |
| **设备自动发现与心跳** | HTTP/1.1 (GET `/api/info`) | TCP `57088` | 局域网网段平滑并发探测与节点状态保活 |
| **即时消息与传输信令** | HTTP/1.1 (POST `/api/message`) | TCP `57088` | 文本对话、文件发送通知、接收/拒绝状态确认 |
| **文件数据流式传输** | HTTP/1.1 (POST `/api/transfer/stream`) | TCP `57088` | 文件二进制流直接写入磁盘，不占用过多系统内存 |

文件默认保存路径：`系统文档目录/LAN Drop/`（支持在设置中自定义目录）
